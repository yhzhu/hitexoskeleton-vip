...
struct
{
    ...
    //server ip address 
    //char ip[32] = "192.168.10.134"; //yhzhu-VM ip
    char ip[32] = "192.168.25.62"; //wsl ip
    int port = 20001;
} settings;
...
// socket section of UI
const mjuiDef defServer[] =
{
    {mjITEM_SECTION,   "Server",        1, NULL,                    "AC"},
    {mjITEM_EDITTXT,   "ip",            2, settings.ip,             ""},
    // {mjITEM_STATIC,    "Value",         2, NULL,                    ""},
    {mjITEM_END}    
};

// walking section of UI
const mjuiDef defWalking[] =
{
    {mjITEM_SECTION,   "Walking",        1, NULL,                    "AD"},
    {mjITEM_BUTTON,    "load model",      2, NULL,                    ""},
    {mjITEM_BUTTON,    "load data",      2, NULL,                    ""},
    {mjITEM_END}    
};
...
void uiEvent(mjuiState* state)
{
        ...
        state->type==mjEVENT_KEY )
    {
        ...
        {
            //vopt.flags[mjVIS_CONVEXHULL] = !vopt.flags[mjVIS_CONVEXHULL];
            //printf("ui event from name:%s,type:%d,itemid:%d\n",it->name,it->type,it->itemid);
            //mjtByte* pd = (mjtByte*)it->pdata;
            //printf("mjVIS_CONVEXHULL value:%d\n",pd[mjVIS_CONVEXHULL]);
...
void simulate(void)
{
    ...
    while( !settings.exitrequest )
    {
        ...
        if( m )
        {
            ...
            simloop_count++;
            if(simloop_count%1000 == 0)printf("simloop_count=%d\n",simloop_count);
        }
...
void setQPos(mjtNum* pos)
{
    mtx.lock();
    //printf("m->nq:%d\n",m->nq);
    if(m)mju_copy(d->qpos, pos, m->nq);
    mtx.unlock();
}
...
void init(void)
{
    ...
    // mjui_add(&ui0, defServer);
    // mjui_add(&ui0, defWalking);
